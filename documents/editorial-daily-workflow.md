...
Текущий цикл: Planner+Architect сверяют ядро 302, реестр и финальные версии; предлагают ровно три структуры; владелец выбирает одну; Writer готовит структурированный текст и устойчивые задания на assets; Designer использует isolated staging; Writer собирает комплект; Validator проверяет фактические итоговые текст и изображения. Один spec и одни hashes сопровождают весь путь. Доработка неизменной темы исправляет точное замечание без полного повторного Planning.

Публикация, индексация, позиции, запуск/оценка ПФ и conversion Measurement не входят в цикл. Публикация остаётся отдельным owner-gate. Непроведённый content/visual review не обозначается `passed`; структура не считается выбранной до явного выбора владельца.
