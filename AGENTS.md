...
- Контролируемый pilot допускает только 2–3 заранее разделённые параллельные read-only research/audit-ветки одной задачи и обязательный синтез Strategy. Они не являются writer/executor и не меняют repository, БД, memory, configuration или Git. `create_thread`, user-owned/sidebar-чаты, параллельные writers и переиспользование завершённого или заблокированного executor запрещены.
