...
def test_demo_login_opens_scenario(monkeypatch) -> None:
    ...
    assert 'class="brand-logo" src="/brand/logo-horizontal-light.png" alt="MH MetricHit"' in response.text
    logo = test_client.get("/brand/logo-horizontal-light.png")
...
def test_manager_persistent_navigation_search_collapse_current_and_mobile(monkeypatch) -> None:
    ...
    with sync_playwright() as playwright:
        ...
        assert logo_box is not None and 202 <= logo_box["width"] <= 208
        assert page.evaluate("getComputedStyle(document.body).backgroundColor") == "rgb(16, 17, 18)"
        ...
        assert page.locator("#nav-tree [data-nav-key]").count() == 1 + len(page.evaluate("shortcutRoots"))
...
def test_manager_centered_workspace_resizable_tree_and_quick_help(monkeypatch) -> None:
    ...
    with sync_playwright() as playwright:
        ...
        page.get_by_role("button", name="Условия бесплатного теста").click()
        assert page.evaluate("c") == "shared-test-details"
