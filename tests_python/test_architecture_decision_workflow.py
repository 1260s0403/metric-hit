...
def test_architecture_decision_workflow_is_idempotent(tmp_path):
    database = tmp_path / "memory.sqlite"
    run_node("scripts/init-memory.mjs", str(database))
    first = run_node("scripts/apply-backend-runtime-python-fastapi.mjs", str(database))
    second = run_node("scripts/apply-backend-runtime-python-fastapi.mjs", str(database))
    ...
    query = """
      ...
      const row = db.prepare(`
        SELECT c.semantic_key, c.status, c.type, c.reviewed_by,
               s.type AS source_type, s.status AS source_status,
               d.type AS document_type, d.status AS document_status, d.version AS document_version,
               dv.status AS version_status, dv.version AS version
        FROM memory_candidates c
        JOIN sources s ON s.id = c.source_id
        JOIN documents d ON d.source_id = s.id
        JOIN document_versions dv ON dv.document_id = d.id
        WHERE c.semantic_key = 'architecture.backend_runtime_python_fastapi'
      `).all();
      console.log(JSON.stringify(row)); db.close();
    """
    rows = json.loads(run_node("--input-type=module", "-e", query, str(database)).stdout)
    assert rows == [{
        "semantic_key": "architecture.backend_runtime_python_fastapi",
        "status": "approved",
        "type": "decision",
        "reviewed_by": "owner",
