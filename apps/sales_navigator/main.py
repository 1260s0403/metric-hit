...
SHARED_DIALOGUE_NODES = {
 "shared-test":{"title":"Бесплатный тест","client":"Хочу сначала попробовать.","manager":"Для нового аккаунта можно один раз получить 1 000 тестовых кликов без оплаты и пополнения. Давайте перейдём в Telegram: там уточним сайт, регион и запросы, а затем поможем с активацией.","hint":"Не обещайте повторный бонус или срок активации.","choices":[{"label":"Перейти в Telegram","next":"shared-telegram"},{"label":"Сначала уточню условия","next":"shared-prices"}]},
 ...
 "shared-objections":{"title":"Возражения","client":"Я сомневаюсь, что нам это подходит.","manager":"Понимаю. Что вызывает больше всего вопросов: принцип работы, условия теста, цена или ожидаемый результат? Разберём один конкретный пункт без обещаний, которых сервис не даёт.","hint":"Сначала выясните настоящее возражение, затем перейдите к нужному общему узлу.","choices":[]}}
DEFAULT_QUICK_HELP = {
...
def app_page(role):
 s=load();is_admin=role=='admin';shortcuts=[{"key":key,"label":value["title"]} for key,value in SHARED_DIALOGUE_NODES.items()]
