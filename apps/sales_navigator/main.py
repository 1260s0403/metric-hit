...
SHARED_DIALOGUE_NODES = {
 ...
 "shared-objections":{"title":"Разобрать сомнения","client":"Я сомневаюсь, что нам это подходит.","manager":"Понимаю. Что вызывает больше всего вопросов: принцип работы, условия теста, цена или ожидаемый результат? Разберём один конкретный пункт без обещаний, которых сервис не даёт.","hint":"Сначала выясните настоящее возражение, затем перейдите к нужному общему узлу.","choices":[{"label":"Как это работает","next":"shared-mechanics"},{"label":"Цены и условия","next":"shared-prices"},{"label":"Ограничения","next":"shared-limits"}]}}
...
def app_page(role):
 s=load();is_admin=role=='admin';shortcuts=[{"key":key,"label":SHARED_DIALOGUE_NODES[key]["title"]} for key in SHARED_DIALOGUE_ROOTS]
