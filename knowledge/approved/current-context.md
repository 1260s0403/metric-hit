...
Сформировано: 2026-08-22T15:44:06.681Z. Этот файл содержит только утверждённую память. Задачи и планы вынесены в отдельный раздел и не являются реализованными фактами.
...
- **План и два пакета принадлежности проектных данных:** Versioned planner детерминированно классифицирует legacy-записи по стабильным ID, явным проектным связям и неизменяемым записям назначения, не используя и не выводя пользовательское содержимое. После проверенных backup/restore 31 запись отнесена к MetricHit и 4 — к управляющему контуру «Ядра» в первом пакете; второй пакет назначил 7 решений MetricHit, 54 обычных решения «Ядру» и 1 решение «Ядру» с append-only исправлением ложной `project_id`-связи без переписывания approved-содержимого. Остались неизменными 304 `unresolved`. Операции транзакционны, идемпотентны, защищены исходными хэшами и точными rollback manifests. Project SQLite, schema, runtime и UI не менялись; фактический перенос и cutover не выполнялись.
