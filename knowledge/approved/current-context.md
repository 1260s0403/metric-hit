...
- **План и первый пакет принадлежности проектных данных:** Versioned planner детерминированно классифицирует legacy-записи по стабильным ID, явным проектным связям и неизменяемым записям назначения, не используя и не выводя пользовательское содержимое. После проверенного backup/restore 31 запись отнесена к MetricHit и 4 — к управляющему контуру «Ядра» через append-only audit; утверждённые записи памяти не переписывались, 366 `unresolved` не затронуты. Операция транзакционна, идемпотентна, защищена исходными хэшами и имеет точный rollback manifest. Project SQLite, schema, runtime и UI не менялись; фактический перенос и cutover не выполнялись.
