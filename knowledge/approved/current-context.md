...
- **Read-only план миграции проектных данных:** Завершён безопасный dry-run этап: versioned planner открывает legacy SQLite только для чтения, детерминированно классифицирует записи по стабильным ID, явным `project_id`/`subproject_id`, утверждённым ролям проектов и структурным связям. Текст и названия не используются и не выводятся. Любая отсутствующая, неизвестная или конфликтующая принадлежность даёт `unresolved` и `readyToMigrate=false`. Данные и schema не изменены, project SQLite не создавались, apply/cutover не выполнялись; фактический перенос требует решения владельца по категориям `unresolved` и отдельного утверждения.
