...
function priorCorrectionCandidateId() {
  const hex = createHash('sha256').update(`metrichit-public-editorial-semantics-indexation-correction:candidate:${semanticKey}:${revision - 1}`).digest('hex');
  return `${hex.slice(0, 8)}-${hex.slice(8, 12)}-4${hex.slice(13, 16)}-a${hex.slice(17, 20)}-${hex.slice(20, 32)}`;
}
...
export function applyPublicEditorialIndexationPfPolicy(databasePath = defaultDatabasePath) {
  ...
  try {
    ...
    if (!prior) throw new Error('Approved indexation/PF-target revision 2 is required before this superseding clarification');
