...
function priorCorrectionCandidateId() {
  const hex = createHash('sha256').update(`metrichit-public-editorial-semantics-indexation-correction:candidate:${semanticKey}:2`).digest('hex');
  return `${hex.slice(0, 8)}-${hex.slice(8, 12)}-4${hex.slice(13, 16)}-a${hex.slice(17, 20)}-${hex.slice(20, 32)}`;
}
...
export function applyPublicEditorialApprovedCoreTargetQueries(databasePath = defaultDatabasePath) {
  ...
  const candidateData = JSON.stringify({
    ...
    supersedes_candidate_id: priorCorrectionCandidateId(),
    ...
    semantic_core: {
      ...
      one_compact_cluster_and_one_intent: true,
    ...
    execution_card: ['selected_cluster', 'primary_target_query', 'secondary_target_queries', 'user_intent', 'platform', 'format', 'core_reference'],
    primary_query_placement: { article: ['h1'], social_post: ['headline', 'opening_paragraph'], natural: true, keyword_stuffing_prohibited: true },
    content_scope: { serves_one_selected_intent: true, unrelated_clusters_mixed: false, duplicate_target_queries_prohibited: true },
    delivery_validation: ['semantic_cluster_selection', 'target_queries_approved_core', 'primary_query_prominence', 'single_cluster_intent', 'geo_demand_verification'],
  ...
  try {
    ...
    const prior = priorRows.find((row) => row.id === priorCorrectionCandidateId() && Number(row.revision) === revision - 1);
    if (!prior) throw new Error('Approved semantic-core revision 2 is required before this superseding clarification');
