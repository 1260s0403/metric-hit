...
export function applyModelRoutingPolicy(databasePath = defaultDatabasePath) {
  ...
  const metadata = JSON.stringify({
    ...
    encoding: 'utf-8',
    authority: 'direct_owner_instruction_implement_all_usage_economy_measures',
  ...
  const candidateData = JSON.stringify({
    ...
    future_agents_md_plan: {
      status: 'usage_economy_implemented',
      ...
      usage_economy: {
        agents_md: 'compact_startup_contract',
        bare_start: 'full_read_only_strategy_audit_only',
        continuation: 'scoped_copy_command_after_chat_finish',
        large_document_reads: 'only_on_scope_rule_change_or_specific_gap',
        usage_review: 'owner_requested_read_only_no_background_processes',
      },
    },
    standard_model: 'GPT-5.6 Terra / Medium',
    revision,
    supersedes: 'ai.model_routing_policy revision 8',
  ...
  try {
    const approvedLineage = new Set([
      ...
      stableUuid('candidate:ai.model_routing_policy:8'),
    ]);
    ...
    created.sources += Number(database.prepare(`
      ...
      VALUES (?, 'owner_decision', ?, ?, ?, 'active', ?, '2026-09-12', 'internal')
    ...
    created.documents += Number(database.prepare(`
      ...
      VALUES (?, 'owner_decision', ?, ?, ?, 'active', ?, ?, '2026-09-12', 'internal', 1)
    ...
    created.versions += Number(database.prepare(`
      ...
      VALUES (?, ?, 'owner_decision', ?, ?, ?, 'active', ?, ?, '2026-09-12', 'internal', 2)
    ...
    created.candidates += Number(database.prepare(`
      ...
      VALUES (?, 'ai_policy', 'ai.model_routing_policy', ?, ?, ?, 'pending', ?, ?, '2026-09-12', 'internal', 1)
    ...
    if (candidate.status === 'pending') {
      database.prepare(`
        ...
        WHERE id = ?
      `).run(owner, reviewedAt, 'Прямое указание владельца: «Реализуй все. Разрешаю», 12.09.2026.', reviewedAt, candidateId);
    }
