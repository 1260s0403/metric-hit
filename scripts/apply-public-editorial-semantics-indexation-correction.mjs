import { createHash } from 'node:crypto';
import { readFileSync } from 'node:fs';
import { dirname, join, resolve } from 'node:path';
import { fileURLToPath, pathToFileURL } from 'node:url';
import { DatabaseSync } from 'node:sqlite';

const repositoryRoot = resolve(dirname(fileURLToPath(import.meta.url)), '..');
const defaultDatabasePath = join(repositoryRoot, 'data', 'database', 'metrichit.db');
const decisionPath = 'knowledge/decisions/public-editorial-semantics-indexation-correction-2026-09-01.md';
const owner = 'owner';
const reviewedAt = '2026-09-01T00:00:00.000Z';
const revision = 2;

const policies = [
  {
    semanticKey: 'content.public_editorial_semantic_core_policy',
    title: 'Семантическое ядро для публичных материалов вне Telegram',
    content: 'Каждый новый публичный пост или статья MetricHit вне Telegram начинается с выбора одного компактного не-навигационного кластера из утверждённого семантического ядра из 145 запросов. Execution card фиксирует кластер, один основной целевой запрос, пользовательский интент, площадку и формат. Основной запрос естественно расположен в H1 статьи либо в заголовке или первом абзаце социального поста; переоптимизация не допускается. Материал раскрывает один выбранный интент и не смешивает несвязанные кластеры; брендовые и навигационные варианты исключены, а геокандидат требует предварительного подтверждения спроса. Delivery validation подтверждает эти локальные факты до готовности нового черновика. Telegram полностью исключён из этого правила. Уже опубликованный архив и существующие черновики не меняются.',
    data: {
      applies_to: ['new_public_posts', 'new_public_articles'],
      platforms: ['vk', 'tenchat', 'article_platforms', 'future_public_editorial_channels'],
      excluded_platforms: ['telegram'],
      semantic_core: { keyword_count: 145, cluster: 'one_compact_non_navigational_cluster', brand_and_navigation_excluded: true, geo_candidates_require_prior_demand_verification: true },
      execution_card: ['selected_cluster', 'primary_target_query', 'user_intent', 'platform', 'format'],
      primary_query_placement: { article: ['h1'], social_post: ['headline', 'opening_paragraph'], natural: true, keyword_stuffing_prohibited: true },
      content_scope: { serves_one_selected_intent: true, unrelated_clusters_mixed: false },
      delivery_validation: ['semantic_cluster_selection', 'primary_query_prominence', 'single_cluster_intent', 'geo_demand_verification'],
    },
  },
  {
    semanticKey: 'content.public_editorial_yandex_indexation_pf_target_policy',
    title: 'Индексация Яндекса и будущий PF-target для публичных материалов вне Telegram',
    content: 'Для каждого нового публичного поста или статьи MetricHit вне Telegram главная редакционная цель — попасть в индекс Яндекса по выбранному утверждённому не-навигационному запросу и компактному семантическому кластеру. Execution card фиксирует только SEO-цель индексации; до публикации и delivery не требуют и не выполняют проверку индексируемости, находимости, URL, статуса или доказательства индексации свежего материала. Только после независимой фактической проверки индексации опубликованный материал может рассматриваться для отдельной owner-approved ПФ-кампании с указанными материалом/URL, бюджетом и scope. Правило не запускает кампанию, не разрешает публикацию, расходы или доступ к аккаунту. Telegram полностью исключён из этого правила. Опубликованный архив и существующие черновики не меняются.',
    data: {
      priority: 'highest_editorial_objective_for_non_telegram_public_materials',
      applies_to: ['new_public_posts', 'new_public_articles'],
      platforms: ['vk', 'tenchat', 'article_platforms', 'future_public_editorial_channels'],
      excluded_platforms: ['telegram'],
      yandex_indexation_objective: { required: true, selected_semantic_core_query_and_cluster_required: true },
      execution_card: ['seo_indexation_objective'],
      delivery_validation: ['semantic_cluster_selection', 'primary_query_prominence', 'single_cluster_intent'],
      prohibited_pre_delivery_checks: ['indexability', 'retrievability', 'target_url_status', 'indexation_status', 'indexation_evidence'],
      post_indexation_pf_target: { eligible_only_after_independently_verified_yandex_indexation: true, campaign_automatic_authorization: false, separate_owner_approval_required_fields: ['material_or_url', 'budget', 'scope'] },
    },
  },
];

function stableUuid(key) {
  const hex = createHash('sha256').update(`metrichit-public-editorial-semantics-indexation-correction:${key}`).digest('hex');
  return `${hex.slice(0, 8)}-${hex.slice(8, 12)}-4${hex.slice(13, 16)}-a${hex.slice(17, 20)}-${hex.slice(20, 32)}`;
}

function assertFields(row, expected, label) {
  if (!row) throw new Error(`Missing ${label}`);
  for (const [column, value] of Object.entries(expected)) if (row[column] !== value) throw new Error(`${label}.${column} differs`);
}

export function applyPublicEditorialSemanticsIndexationCorrection(databasePath = defaultDatabasePath) {
  const bytes = readFileSync(join(repositoryRoot, decisionPath));
  const documentContent = new TextDecoder('utf-8', { fatal: true }).decode(bytes);
  const title = 'Уточнение семантики и индексации публичных материалов';
  const metadata = JSON.stringify({ path: decisionPath, bytes: bytes.length, sha256: createHash('sha256').update(bytes).digest('hex'), encoding: 'utf-8', authority: 'direct_owner_confirmation', decision_date: '2026-09-01', revision });
  const sourceId = stableUuid(`source:${decisionPath}:${revision}`);
  const documentId = stableUuid(`document:${decisionPath}:${revision}`);
  const versionId = stableUuid(`version:${decisionPath}:${revision}`);
  const database = new DatabaseSync(databasePath);
  const created = { sources: 0, documents: 0, versions: 0, candidates: 0 };
  database.exec('PRAGMA foreign_keys = ON; BEGIN IMMEDIATE;');
  try {
    for (const policy of policies) {
      const candidateId = stableUuid(`candidate:${policy.semanticKey}:${revision}`);
      const priorRows = database.prepare("SELECT id,status,coalesce(json_extract(data_json, '$.revision'), 0) AS revision FROM memory_candidates WHERE semantic_key=? AND status IN ('pending','approved') AND id<>?").all(policy.semanticKey, candidateId);
      const competing = priorRows.filter((row) => row.status !== 'approved' || Number(row.revision) >= revision);
      if (competing.length) throw new Error(`Semantic duplicate or evolution requires a single approved prior revision for ${policy.semanticKey}`);
      const conflict = database.prepare("SELECT id FROM memory_conflicts WHERE status='open' AND (candidate_id=? OR existing_memory_item_id IN (SELECT id FROM memory_items WHERE semantic_key=?))").get(candidateId, policy.semanticKey);
      if (conflict) throw new Error(`Open memory conflict blocks ${policy.semanticKey}`);
    }
    created.sources += Number(database.prepare("INSERT OR IGNORE INTO sources (id,type,title,content,data_json,status,author,valid_at,access_level) VALUES (?, 'owner_decision', ?, ?, ?, 'active', ?, '2026-09-01', 'internal')").run(sourceId, title, `Repository file: ${decisionPath}`, metadata, owner).changes);
    created.documents += Number(database.prepare("INSERT OR IGNORE INTO documents (id,type,title,content,data_json,status,source_id,author,valid_at,access_level,version) VALUES (?, 'owner_decision', ?, ?, ?, 'active', ?, ?, '2026-09-01', 'internal', 1)").run(documentId, title, documentContent, metadata, sourceId, owner).changes);
    created.versions += Number(database.prepare("INSERT OR IGNORE INTO document_versions (id,document_id,type,title,content,data_json,status,source_id,author,valid_at,access_level,version) VALUES (?, ?, 'owner_decision', ?, ?, ?, 'active', ?, ?, '2026-09-01', 'internal', 1)").run(versionId, documentId, title, documentContent, metadata, sourceId, owner).changes);
    for (const policy of policies) {
      const candidateId = stableUuid(`candidate:${policy.semanticKey}:${revision}`);
      const data = JSON.stringify({ ...policy.data, revision, supersedes_semantic_revision: revision - 1, supersedes_candidate_id: stableUuid(`candidate:${policy.semanticKey}`), evidence: { path: decisionPath } });
      created.candidates += Number(database.prepare("INSERT OR IGNORE INTO memory_candidates (id,type,semantic_key,title,content,data_json,status,source_id,author,valid_at,access_level,version) VALUES (?, 'editorial_rule', ?, ?, ?, ?, 'pending', ?, ?, '2026-09-01', 'internal', 1)").run(candidateId, policy.semanticKey, policy.title, policy.content, data, sourceId, owner).changes);
      if (database.prepare('SELECT status FROM memory_candidates WHERE id=?').get(candidateId)?.status === 'pending') database.prepare("UPDATE memory_candidates SET status='approved', reviewed_by=?, reviewed_at=?, review_note=?, updated_at=?, version=version+1 WHERE id=?").run(owner, reviewedAt, 'Одобрено прямым уточнением владельца MetricHit от 01.09.2026; заменяет редакцию 1.', reviewedAt, candidateId);
      assertFields(database.prepare('SELECT * FROM memory_candidates WHERE id=?').get(candidateId), { type: 'editorial_rule', semantic_key: policy.semanticKey, title: policy.title, content: policy.content, data_json: data, source_id: sourceId, status: 'approved', reviewed_by: owner, reviewed_at: reviewedAt }, `${policy.semanticKey} correction candidate`);
    }
    assertFields(database.prepare('SELECT * FROM documents WHERE id=?').get(documentId), { content: documentContent, data_json: metadata, source_id: sourceId, version: 1 }, 'editorial semantics/indexation correction document');
    database.exec('COMMIT');
    return { databasePath, created, sourceId, documentId, versionId, candidateIds: policies.map((policy) => stableUuid(`candidate:${policy.semanticKey}:${revision}`)) };
  } catch (error) { database.exec('ROLLBACK'); throw error; } finally { database.close(); }
}

if (process.argv[1] && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) console.log(`Applied public editorial semantics/indexation correction: ${JSON.stringify(applyPublicEditorialSemanticsIndexationCorrection(process.argv[2] ? resolve(process.argv[2]) : defaultDatabasePath))}`);
