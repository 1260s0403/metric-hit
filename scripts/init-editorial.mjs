...
function verifyPipelineProfiles(database) {
...
export function updateEditorialInfrastructure(databasePath = defaultProjectDatabasePath, options = {}) {
  ...
  try {
    ...
    const profiles = verifyPipelineProfiles(database);
...
if (isMainModule()) {
  try {
    ...
    if (updateMode) {
      const result = updateEditorialInfrastructure(explicitPath ? resolve(explicitPath) : defaultProjectDatabasePath);
