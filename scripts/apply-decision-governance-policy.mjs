...
export function applyDecisionGovernancePolicy(databasePath = defaultDatabase) {
  ...
  content += ' Контролируемый multi-agent research pilot сохраняет 2–3 независимые параллельные read-only research/audit-ветки одной задачи с обязательным synthesis Strategy. Отдельно controlled parallel execution v1 разрешает до двух независимых mutation/code/data/config-задач; внутри каждого набора изменений по-прежнему один named writer/executor, одна ветка, один commit и чистый Git.';
  ...
  const policyData = JSON.stringify({
    ...
    execution: {
      ...
      controlled_parallel_execution_v1: {
        enabled: true, maximum_active_writers: 2, canonical_worktree_allowed: false,
  ...
  const operationsContent = 'SERVER и C:\\MetricHit\\workspace остаются primary workspace MetricHit. Strategy — read-only human-facing router. Controlled parallel execution v1 допускает максимум два независимых writer-а: каждый работает в отдельном неканоническом Git worktree и ветке, объявляет точные path/SQLite/shared resources и имеет один commit. Пересечения, третья задача и core/shared resources блокируются fail-closed; разные project SQLite совместимы; central control-plane, context pack, memory/policy и integration сериализованы. Read-only research pilot 2–3 веток сохраняется. Fast path, clean Git, проверки, Sol/Luna gates, owner-gates и managed sandbox сохраняются.';
  ...
  const operationsData = JSON.stringify({
    ...
    repository_mutation: { responsible_executors_per_change_set: 1, maximum_parallel_writers: 2, commits_per_change_set: 1, in_scope_owner_request_is_authorization: true, redundant_intermediate_confirmation_required: false, parallel_writers_allowed: true, resource_leases_required: true, integration_serialized: true },
