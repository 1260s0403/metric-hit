...
export function applyDecisionGovernancePolicy(databasePath = defaultDatabase) {
  ...
  content += ' Сразу после запуска native executor-а Strategy сообщает владельцу имя executor-а и точный scope. После завершения Strategy сам публикует краткий итог: готово/blocked, commit (если есть), проверки, git status и blocker (если есть). Для этого не вводятся периодические статусы, scheduler, UI или новая функциональность; остальной workflow не меняется.';
