...
export function applyDecisionGovernancePolicy(databasePath = defaultDatabase) {
  ...
  const policyData = JSON.stringify({
    ...
    implementation_authorization: {
      ...
      finish_task_command: { meaning: 'prepare_for_manual_new_chat', completes_pending_result_first: true, cli: 'chat-finish', delivered_context_pack_required: true, preserves_old_chat: true, output: 'single_fenced_text_scoped_startup_command_only' },
  ...
  try {
    ...
    if (candidate.status === 'pending') {
      db.prepare(`UPDATE memory_candidates SET status='approved',reviewed_by=?,reviewed_at=?,review_note=?,updated_at=?,version=version+1 WHERE id=?`).run(owner, reviewedAt, 'Граница уточнена прямым решением владельца от 05.09.2026; остальные утверждённые правила сохранены.', reviewedAt, candidateId);
    }
    ...
    if (operationsCandidate.status === 'pending') {
      db.prepare(`UPDATE memory_candidates SET status='approved',reviewed_by=?,reviewed_at=?,review_note=?,updated_at=?,version=version+1 WHERE id=?`).run(owner, reviewedAt, 'Граница уточнена прямым решением владельца от 05.09.2026; остальные утверждённые правила сохранены.', reviewedAt, operationsCandidateId);
    }
