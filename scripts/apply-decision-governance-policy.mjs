...
const reviewedAt = '2026-09-05T07:43:00.000Z';
const owner = 'owner';
const revision = 36;
const operationsRevision = 23;
...
export function applyDecisionGovernancePolicy(databasePath = defaultDatabase) {
  ...
  const policyData = JSON.stringify({
    ...
    execution: {
      ...
      small_change_fast_path: {
        ...
        strategy_full_startup_completed_once: false,
        consultation_and_read_only_review: { writer_required: false, execution_card_required: false, full_strategy_startup_required: false, reads: 'verified_scope_context_or_named_files_and_direct_dependencies' },
    ...
    implementation_authorization: {
      ...
      direct_instruction_aliases: ['сделай', 'реализуй', 'доделай', 'исправь', 'делай сам'],
      includes: ['implementation', 'tests', 'ordinary_git_staging', 'one_commit', 'managed_sequential_integration'],
      redundant_intermediate_confirmation_required: false,
      second_owner_lifecycle_command_required: false,
      approved_decision_recording_requires_reapproval: false,
      executor_claims_internal_handoff_and_lease: true,
      missing_initial_handoff_is_blocker: false,
      finish_task_command: { meaning: 'prepare_for_manual_new_chat', completes_pending_result_first: true, cli: 'chat-finish', delivered_context_pack_required: true, preserves_old_chat: true, output: 'single_fenced_text_scoped_startup_command_only', separate_from_delivery: true },
  ...
  const operationsData = JSON.stringify({
    ...
    execution_workspace_boundary: { root: 'C:\\MetricHit\\worktrees', registered_same_repository_only: true, verify: ['registration', 'branch', 'head', 'clean_state', 'no_conflicting_lease'], reuse_same_line: true, new_chat_alone_requires_new_worktree: false, independent_clones_allowed: false, global_settings_changed: false },
    primary_workspace: 'C:\\MetricHit\\workspace',
    strategy: { owner_visible: true, read_only: true, permanent_project_chat: true },
    repository_mutation: { responsible_executors_per_change_set: 1, maximum_parallel_writers: 4, commits_per_change_set: 1, in_scope_owner_request_is_authorization: true, redundant_intermediate_confirmation_required: false, second_owner_lifecycle_command_required: false, executor_claims_internal_handoff_and_lease: true, parallel_writers_allowed: true, resource_leases_required: true, integration_serialized: true },
  ...
  try {
    ...
    if (candidate.status === 'pending') {
      db.prepare(`UPDATE memory_candidates SET status='approved',reviewed_by=?,reviewed_at=?,review_note=?,updated_at=?,version=version+1 WHERE id=?`).run(owner, reviewedAt, 'Прямое подтверждение владельца после перечисления пяти уточнений и platform risk: «Делай )», 05.09.2026.', reviewedAt, candidateId);
    }
    ...
    if (operationsCandidate.status === 'pending') {
      db.prepare(`UPDATE memory_candidates SET status='approved',reviewed_by=?,reviewed_at=?,review_note=?,updated_at=?,version=version+1 WHERE id=?`).run(owner, reviewedAt, 'Прямое подтверждение владельца после перечисления пяти уточнений и platform risk: «Делай )», 05.09.2026.', reviewedAt, operationsCandidateId);
    }
