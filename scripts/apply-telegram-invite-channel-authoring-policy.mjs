...
export function applyTelegramInviteChannelAuthoringPolicy(databasePath = defaultDatabasePath) {
  ...
  const content = 'Invite-канал MetricHit ведёт читателя в основной Telegram-канал и не дублирует его длинные разборы. Каждый новый пост содержит 600–750 знаков вместе с точным полным футером, написан plain text без Markdown, эмодзи, стрелок и декоративных списков. Один пост раскрывает одну конкретную узнаваемую ситуацию, даёт один полезный принцип и краткую причину продолжить тему в основном канале, не исчерпывая глубокий материал. Автор чередует заходы и форматы, не использует универсальный заполнитель, опирается только на подтверждённые факты, не придумывает метрики, кейсы и обновления и не раскрывает механику бота. Футер ровно один: основной канал https://t.me/mtr_hit, сайт https://go.mtrhit.ru/ и поддержка https://t.me/Metric_Hit. Публикация, запуск бота или расписания, сеть, credentials и привязка канала этим решением не разрешаются.';
  const data = JSON.stringify({ platform: 'telegram', channel_role: 'invite_to_main_channel', character_count_including_footer: { minimum: 600, maximum: 750 }, plain_text_only: true, decorative_lists_prohibited: true, structure: ['recognizable_situation', 'one_useful_reframe_or_principle', 'main_channel_bridge'], full_footer: ['https://t.me/mtr_hit', 'https://go.mtrhit.ru/', 'https://t.me/Metric_Hit'], prohibited: ['invented_metrics_or_cases', 'unverified_search_updates', 'search_bot_mechanics_disclosure', 'publication_or_runtime_actions'], evidence: { path: decisionPath } });
  ...
  const sourceId = stableUuid(`source:${decisionPath}`); const documentId = stableUuid(`document:${decisionPath}`);
  const versionId = stableUuid(`version:${decisionPath}:1`); const candidateId = stableUuid(`candidate:${semanticKey}`);
  ...
  try {
    ...
    if (duplicate) throw new Error('Semantic duplicate blocks Telegram invite authoring policy');
