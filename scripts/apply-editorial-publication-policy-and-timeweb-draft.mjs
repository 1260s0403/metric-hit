...
export function applyEditorialPublicationPolicyAndTimewebDraft(databasePath = defaultDatabasePath) {
  ...
  const contourId = stableUuid('candidate:editorial.metrichit_contour_and_research_mvp:revision:1');
  ...
  try {
    ...
    if (contourRevisions.some(({ revision }) => Number(revision) !== 0)) throw new Error('Unexpected editorial contour revision blocks revision 1');
