...
const articleProhibitionsReviewedAt = '2026-08-31T00:00:00.000Z';
...
export function applyEditorialPublicationPolicyAndTimewebDraft(databasePath = defaultDatabasePath) {
  ...
  const botMechanicsTitle = 'Статьи MetricHit: не раскрывать механику бота';
  const botMechanicsContent = 'Никогда не описывать, как работает бот, и не раскрывать механику его действий в каждой статье MetricHit на любой площадке.';
  const noGuaranteesTitle = 'Статьи MetricHit: не писать об отсутствии гарантий';
  const noGuaranteesContent = 'Никогда не писать об отсутствии гарантий результата, позиций, трафика или лидов в каждой статье MetricHit на любой площадке.';
  const publicNoGuaranteesTitle = 'Публичные материалы MetricHit: не упоминать отсутствие гарантий';
  const publicNoGuaranteesContent = 'Во всех новых публичных материалах MetricHit — статьях, TenChat, VK, Telegram и других открытых каналах — никогда не упоминать формулировку «позиции не гарантируются» и её смысловые варианты: отсутствие гарантий результата, позиций, трафика или лидов. Правило регулирует только публичную редакционную подачу: оно не отменяет внутренние продуктовые факты и обязательные юридические раскрытия, а также не требует ретроспективно переписывать уже опубликованные материалы.';
  ...
  const botMechanicsData = JSON.stringify({ applies_to: ['articles'], platforms: ['all'], prohibited: ['bot_operation_description', 'bot_action_mechanics'], evidence: { path: decisionPath, owner_clarification_date: '2026-08-31' } });
  const noGuaranteesData = JSON.stringify({ applies_to: ['articles'], platforms: ['all'], prohibited: ['absence_of_result_guarantees', 'absence_of_position_guarantees', 'absence_of_traffic_guarantees', 'absence_of_lead_guarantees'], evidence: { path: decisionPath, owner_clarification_date: '2026-08-31' } });
  const publicNoGuaranteesData = JSON.stringify({ applies_to: ['new_public_materials'], platforms: ['article', 'tenchat', 'vk', 'telegram', 'other_public_channels'], prohibited_exact_phrase: 'позиции не гарантируются', prohibited_semantic_variants: ['absence_of_result_guarantees', 'absence_of_position_guarantees', 'absence_of_traffic_guarantees', 'absence_of_lead_guarantees'], exceptions: ['internal_product_facts', 'mandatory_legal_disclosures', 'no_retroactive_rewrite_of_existing_published_materials'], evidence: { path: decisionPath, owner_clarification_date: '2026-09-01' } });
  ...
  const botMechanicsId = stableUuid('candidate:content.article_prohibit_bot_mechanics');
  const noGuaranteesId = stableUuid('candidate:content.article_prohibit_no_guarantees');
  const publicNoGuaranteesId = stableUuid('candidate:content.public_prohibit_no_guarantees');
  ...
  try {
    ...
    created.candidates += Number(database.prepare("INSERT OR IGNORE INTO memory_candidates (id,type,semantic_key,title,content,data_json,status,source_id,author,valid_at,access_level,version) VALUES (?, 'editorial_rule', 'content.article_prohibit_bot_mechanics', ?, ?, ?, 'pending', ?, ?, '2026-08-31', 'internal', 1)").run(botMechanicsId, botMechanicsTitle, botMechanicsContent, botMechanicsData, sourceId, owner).changes);
    created.candidates += Number(database.prepare("INSERT OR IGNORE INTO memory_candidates (id,type,semantic_key,title,content,data_json,status,source_id,author,valid_at,access_level,version) VALUES (?, 'editorial_rule', 'content.article_prohibit_no_guarantees', ?, ?, ?, 'pending', ?, ?, '2026-08-31', 'internal', 1)").run(noGuaranteesId, noGuaranteesTitle, noGuaranteesContent, noGuaranteesData, sourceId, owner).changes);
    created.candidates += Number(database.prepare("INSERT OR IGNORE INTO memory_candidates (id,type,semantic_key,title,content,data_json,status,source_id,author,valid_at,access_level,version) VALUES (?, 'editorial_rule', 'content.public_prohibit_no_guarantees', ?, ?, ?, 'pending', ?, ?, '2026-09-01', 'internal', 1)").run(publicNoGuaranteesId, publicNoGuaranteesTitle, publicNoGuaranteesContent, publicNoGuaranteesData, sourceId, owner).changes);
    ...
    approve(database, botMechanicsId, articleProhibitionsReviewedAt);
    approve(database, noGuaranteesId, articleProhibitionsReviewedAt);
    approve(database, publicNoGuaranteesId, '2026-09-01T00:00:00.000Z');
    ...
    assertFields(database.prepare('SELECT * FROM memory_candidates WHERE id=?').get(botMechanicsId), { type: 'editorial_rule', semantic_key: 'content.article_prohibit_bot_mechanics', title: botMechanicsTitle, content: botMechanicsContent, data_json: botMechanicsData, source_id: sourceId, status: 'approved', reviewed_by: owner, reviewed_at: articleProhibitionsReviewedAt }, 'bot mechanics prohibition');
    assertFields(database.prepare('SELECT * FROM memory_candidates WHERE id=?').get(noGuaranteesId), { type: 'editorial_rule', semantic_key: 'content.article_prohibit_no_guarantees', title: noGuaranteesTitle, content: noGuaranteesContent, data_json: noGuaranteesData, source_id: sourceId, status: 'approved', reviewed_by: owner, reviewed_at: articleProhibitionsReviewedAt }, 'no guarantees prohibition');
    assertFields(database.prepare('SELECT * FROM memory_candidates WHERE id=?').get(publicNoGuaranteesId), { type: 'editorial_rule', semantic_key: 'content.public_prohibit_no_guarantees', title: publicNoGuaranteesTitle, content: publicNoGuaranteesContent, data_json: publicNoGuaranteesData, source_id: sourceId, status: 'approved', reviewed_by: owner, reviewed_at: '2026-09-01T00:00:00.000Z' }, 'public no guarantees prohibition');
