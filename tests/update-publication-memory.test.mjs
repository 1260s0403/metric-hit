...
test('owner-confirmed Oborot article records its cover and exact PF target queries without recording campaign execution', () => {
  const directory = mkdtempSync(join(tmpdir(), 'metrichit-oborot-business-readiness-publication-'));
  const databasePath = join(directory, 'memory.sqlite');
  try {
    execFileSync(process.execPath, [resolve('scripts/init-memory.mjs'), databasePath]);
    const first = updatePublicationMemory(databasePath, oborotBusinessSiteLaunchReadinessPublicationUpdate);
    const second = updatePublicationMemory(databasePath, oborotBusinessSiteLaunchReadinessPublicationUpdate);
    assert.deepEqual(first.created, { sources: 1, documents: 1, versions: 1, candidates: 1 });
    assert.deepEqual(second.created, { sources: 0, documents: 0, versions: 0, candidates: 0 });
    const readOnly = new DatabaseSync(databasePath, { readOnly: true });
    const current = readOnly.prepare(`SELECT content,data_json,status FROM memory_candidates
      WHERE semantic_key='publication.oborot_business_site_launch_readiness_2026_09_02'`).get();
    const data = JSON.parse(current.data_json);
    assert.equal(current.status, 'approved');
    assert.equal(data.publication_status, 'owner_confirmed_published');
    assert.equal(data.canonical_url, null);
    assert.equal(data.public_url_status, 'not_provided_by_owner');
    assert.equal(data.verified_facts.source_draft_path, 'work/articles/drafts/2026-09-02-oborot-pf-site-readiness-business.md');
    assert.equal(data.verified_facts.cover_asset_path, 'work/articles/assets/2026-09-02-oborot-pf-site-readiness-business-cover.png');
    assert.equal(data.verified_facts.cover_asset_sha256, '958998fa940536ef2858f55d966ce05462d736d561fee6b6323fec458a1e00b9');
    assert.deepEqual(data.verified_facts.pf_promotion_target_queries, [
      'накрутка ПФ сайта', 'накрутка поведенческих факторов сайта', 'продвижение сайта поведенческими факторами',
      'сервис поведенческих факторов для бизнеса', 'платформа для управления поведенческими факторами',
      'инструмент для продвижения в Яндексе', 'как выбрать запросы для накрутки ПФ',
      'какие запросы продвигать ПФ', 'с каких запросов начинать продвижение сайта',
      'как распределить запросы по страницам', 'один запрос — одна страница SEO',
      'релевантная страница для запроса', 'как запустить накрутку ПФ', 'накрутка ПФ самостоятельно',
      'настройка проекта ПФ', 'когда начинать накрутку ПФ', 'что проверить перед накруткой ПФ',
