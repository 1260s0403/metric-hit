...
def _page(token: str, focus_task: str | None, view: str) -> str:
    ...
    page = re.sub(r'(<button data-view="(?:artem|idea|tasks|memory)") class="active"', r'\1', page)
    page = page.replace(
        f'<button data-view="{selected}">',
        f'<button data-view="{selected}" class="active" aria-current="page">',
    )
    page = page.replace("let view=focusTask?'tasks':'artem'", f"let view={json.dumps(selected)}")
    if selected in {"tasks", "memory"}:
        page = page.replace('<div id="knowledge">', '<div id="knowledge" class="hidden">')
    return page
